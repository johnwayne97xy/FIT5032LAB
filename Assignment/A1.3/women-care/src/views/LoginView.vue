...
const validateLoginName = (blur) => {
  if (loginData.value.username.trim().length < 3) {
    if (blur) loginErrors.value.username = 'Name must be at least 3 characters.'
  } else {
...
const validateLoginPassword = (blur) => {
  if (!loginData.value.password) {
    if (blur) loginErrors.value.password = 'Please enter your password.'
  } else {
